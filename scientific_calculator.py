...
class ScientificCalculator(QDialog):  # Changed QWidget to QDialog
    ...
    def evaluate_expression(self, expression):
        # Replace custom symbols with actual Python functions
        ...
        expression = expression.replace("∛x", "math.pow(x, 1/3)")
        expression = expression.replace("10^x", "10**")
        expression = expression.replace("log", "math.log10")
        
        # Handle trigonometric functions with or without radians
        ...
        # If in "2nd" mode, invert the trigonometric functions (e.g., asin, acos, atan)
        if self.is_second_function:
            ...
            expression = expression.replace("cot", "math.acot")
            expression = expression.replace("sec", "math.asec")
            expression = expression.replace("csc", "math.acsc")
        
        # Handle basic operators, replace them if needed
